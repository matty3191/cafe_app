...
from orders_crud_functions import (
    create_new_order,
    delete_order,
    update_order,
    update_order_status,
...
from user_interface_layer import (
    ...
    enumerate_order_status_list,
    enumerate_products_list,
    main_menu_options,
    order_menu_interface,
    products_menu_interface,
)
from utility_functions import (
    courier_list_of_dicts,
    order_list_of_dicts,
        ...
        if p_command > 4 or p_command < 0:
            ...
            while True:
                ...
                try:
                    ...
                    break
            create_new_product(
                product_list_of_dicts, new_product_name, new_product_price
            )
            ...
            while True:
                ...
                if name_or_price.lower() == "price":
                    try:
                        ...
                        print(
                            "Error: When selecting price, input must be a decimal number\n"
                        )
                        continue
                update_product(
                    product_list_of_dicts,
    ...
    while True:
        ...
        if c_command > 4 or c_command < 0:
            ...
            if new_courier_phone == "" or len(str(new_courier_phone)) != 10:
                ...
                continue
            create_new_courier(
                courier_list_of_dicts, new_courier_name, new_courier_phone
            )
            ...
            if name_or_phone.lower() == "phone":
                if (
                    len(new_courier_information) != 11
                ):  # must be str as int starting with 0 drops the 0
                    print(
                        "Error: Input cannot be blank must be an 11 digit long whole number1\n"
                    )
                    continue
            ...
            update_courier(
                courier_list_of_dicts,
                update_courier_index,
                name_or_phone,
                new_courier_information,
            )
            print("Courier succesfuly updated\n")

        elif c_command == 4:  # delete courier
            enumerate_courier_list(courier_list_of_dicts)
            delete_index_value = input(
                "Please enter the index of the courier you wish to delete\n"
            )
            try:
    ...
    while True:
        ...
        if order_command > 5 or order_command < 0:
            ...
            if (
                ...
                or len(new_order["customer_phone"]) != 11
            ):  # same issue as above having to be an str
                print("Error: Field cannot be blank and must contain 11 numbers\n")
                continue
            enumerate_courier_list(courier_list_of_dicts)
            new_order["assigned_courier"] = input(
                "Please select the index of a courier to assign to this order\n"
            )
            try:
            ...
            enumerate_products_list(product_list_of_dicts)
            new_order["items"] = input(
                "Select the items to add to the order, mulitple items are comma separated:\n"
            )
            ...
            enumerate_order_list(order_list_of_dicts)
            order_status_index_update = input(
                "Please enter the index of the order to update the status of\n"
            )
            try:
            ...
            update_order_status(
                order_list_of_dicts, order_status_index_update, new_order_status_value
            )
            ...
            if order_key not in order_list_of_dicts[order_index]:
            ...
            enumerate_order_list(order_list_of_dicts)
            delete_order_index = input(
                "Please enter the number of the order you would like to delete from the list:\n"
            )
            try:
