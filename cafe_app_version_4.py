...
from file_utils import save_courier_files, save_product_files
from file_utils import product_file_list, courier_file_list, order_file_list
from order_views import enumerate_order_list, view_orders
from orders import create_new_order, delete_order, update_order, update_order_status
...
5) Remove existing order.\n"""))
    while order_command != 0:
        if order_command >5 or order_command <0:
            ...
            orders_menu()
        if order_command == 1:
            view_orders(orders_menu, order_file_list)
        if order_command == 2:
            create_new_order(courier_file_list, order_file_list, orders_menu)
        if order_command == 3:
            update_order_status()
        if order_command == 4:
            ...
            update_order(order_file_list, order_index, order_key, new_order_value)
